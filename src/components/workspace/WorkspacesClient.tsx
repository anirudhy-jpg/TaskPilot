...
import { Briefcase, Plus, Loader2, ArrowRight, ShieldCheck, User, Trash2 } from "lucide-react"
...
export function WorkspacesClient({
        ...
        <div className="shrink-0 pl-2 flex items-center gap-2">
          {!isOwner && (
            <Button
              ...
              className="border-red-200 hover:bg-red-50 text-red-600 cursor-pointer text-[10px] font-bold px-2.5 py-1 rounded-lg h-7"
            >
              {leavingId === ws.id ? (
  ...
  return (
    <div className={`space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300 select-none relative transition-all duration-300 ${isPending ? "opacity-75 pointer-events-none filter blur-[0.3px]" : ""}`}>
      ...
      {errorMsg && (
        <div className="p-3 rounded-lg bg-red-50 border border-red-100 text-xs text-red-600 flex items-center justify-between">
          <span>{errorMsg}</span>
        </div>
      )}
