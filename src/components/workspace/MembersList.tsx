...
export function MembersList({
  ...
  // Filter out the owner of the workspace so only other team members are displayed if requested,
  // but wait, the project requires showing all workspace members, including owner/admin/members.
  // Wait, let's keep all workspace members but sort them by role (owner -> admin -> member) or created date.
                        ...
                        {activeMenuId === member.id && (
                          <div className="absolute right-0 mt-1 w-40 bg-white border border-amber-900/10 rounded-xl shadow-lg py-1 z-10 animate-in fade-in zoom-in-95 duration-100">
                            <button
                              ...
                              className="w-full px-3.5 py-2 text-left text-xs font-semibold text-red-600 hover:bg-red-50 transition-colors cursor-pointer flex items-center gap-2"
                            >
                              <Trash2 size={13} />
                              Remove member
                            </button>
