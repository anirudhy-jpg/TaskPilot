...
export async function POST(req: NextRequest) {
  try {
    ...
    if (invite.email.toLowerCase() !== user.email.toLowerCase()) {
      return NextResponse.json(
        { error: "Your email does not match this invitation" },
        { status: 403 },
      );
    }
    ...
    if (invite.project_id) {
      ...
      if (!existingProjMember) {
        ...
        if (projMemberErr) {
          console.warn(
            "Project member insertion skipped or handled by trigger:",
            projMemberErr.message,
          );
        }
      }
    }
    ...
    try {
      ...
      console.error(
        "Failed to set active_workspace_id cookie in API:",
        cookieErr,
      );
    }
