        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            {task.subtasks && task.subtasks.length > 0 && (
              <span className="inline-flex items-center gap-1 text-[9px] font-bold text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded-full" title={`${task.subtasks.filter((st: any) => st.completed || st.status === 'done').length}/${task.subtasks.length} Subtasks completed`}>
                ...
                {task.subtasks.filter((st: any) => st.completed || st.status === 'done').length}/{task.subtasks.length}
              </span>
            )}
          </div>
