...
export function useProjectBoard({
  ...
  const currentProjectMembers = members.filter((m) =>
    projectMemberUserIds.includes(m.userId) || m.userId === activeProject?.createdBy
  );
  const eligibleMembers = members.filter(
    (m) => !projectMemberUserIds.includes(m.userId) && m.userId !== activeProject?.createdBy
  );
  ...
  const handleUpdateProject = (
    ...
    startTransition(async () => {
      ...
      const { updateProjectAction } =
        await import("../actions/update-project.action");
