    ...
    <aside className={asideClasses}>
      ...
      <div className="px-3 mb-3 flex items-center justify-between">
        <div>
          ...
          <div className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate max-w-[140px]" title={workspaceName}>
            {workspaceName}
          </div>
        </div>
        <Link
      ...
      {/* Divider */}
      <div className="border-b border-amber-900/10 mb-3" />
              ...
              className={`
                ...
                ${
                  active
                    ...
                    : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-105 hover:bg-white/80 dark:hover:bg-white/10 hover:translate-x-1"
                }
              `}
            >
              <Icon
                size={16}
                className={active ? "text-slate-950" : "text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-300"}
              />
              <span>{label}</span>
            </Link>
          )
            ...
            {projects.map((project) => {
              ...
              return (
                <div key={project.id} className="flex flex-col mb-1">
                  <div className="flex items-center justify-between w-full rounded-xl text-xs font-medium transition-all duration-200 hover:bg-white/50 dark:hover:bg-white/5">
                    <Link
                      href={`/projects/${project.id}`}
                      onClick={handleLinkClick}
                      className={`flex-1 flex items-center gap-2 px-3 py-2.5 rounded-l-xl ${
                        isProjectActive
                          ? "text-amber-955 font-extrabold bg-gradient-to-r from-amber-50 to-white/70 dark:from-amber-950/20 dark:to-white/5 border-l-3 border-amber-600 shadow-3xs"
                          : "text-slate-600 dark:text-slate-400 hover:text-slate-955 dark:hover:text-slate-200 hover:translate-x-0.5"
                      }`}
                    >
                      <FolderKanban
                        size={14}
                      ...
                      <div className="flex flex-col min-w-0">
                        ...
                        {(project.creatorName || project.creatorEmail) && (
                          <span className="text-[9px] text-slate-450 dark:text-slate-500 font-normal truncate mt-0.5" title={`Created by: ${project.creatorEmail}`}>
                  ...
                  {isExpanded && (
                    <div className="pl-5 flex flex-col gap-1 border-l border-amber-900/5 ml-5 py-1.5">
                      {project.tasks && project.tasks.length > 0 ? (
                        project.tasks.map((task) => {
                          ...
                          return (
                            <Link
                              ...
                              href={`/projects/${project.id}`}
                              onClick={handleLinkClick}
                              className="flex items-center gap-2 px-2 py-1 text-[11px] text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 rounded transition-colors truncate"
                            >
                              <span
                        ...
                        <span className="text-[10px] text-slate-450 italic px-2 py-0.5">
                          No tasks
                        </span>
                      )}
                    </div>
...
export function Sidebar(props: SidebarProps) {
  return (
    <Suspense
      fallback={
        <div className={props.variant === "mobile" ? "w-full h-full bg-white dark:bg-[#131211]" : "w-64 border-r border-amber-900/5 bg-white/45 backdrop-blur-lg p-5"} />
      }
    >
